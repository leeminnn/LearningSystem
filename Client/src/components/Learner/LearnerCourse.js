...
function LearnerCourse({ match }) {
  ...
  async function getQuestions(e) {
    try {
      const onSubmit = await axios({
        method: "post",
        url: "http://3.18.143.100:5002/get_questions",
        data: {
  ...
  async function getSectionList() {
    try {
      const onSubmit = await axios({
        method: "post",
        url: "http://3.18.143.100:5002/get_user_sections",
        data: {
          class_id: match.params.id,
          course_id: course_id,
          emp_id: emp_Id,
  ...
  async function getProgress() {
    try {
      const onSubmit = await axios({
        method: "post",
        url: "http://3.18.143.100:5000/learner_progress",
            ...
            {sectionList.map((entry) => (
              ...
              >
                <ListItemText
                  ...
                  primary={"Section " + entry.section_id}
                />
              </ListItem>
            ))}
            <Divider light />
            {progress == 100 && (
              <ListItem
                ...
                onClick={() => {
                  ...
                  setDesc(
                    "You have 30 minutes to complete this quiz which would determine your final grade for this module."
                  );
                  setQuizID({
                    quiz_id: parseInt(
                      course_id.toString() + match.params.id.toString()
                    ),
                    total_marks: 50,
                    time: 4200,
                  });
