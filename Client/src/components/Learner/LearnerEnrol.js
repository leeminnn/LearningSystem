...
function LearnerEnrol() {
  ...
  async function enrollClass(e, entry) {
    try {
      const onSubmit = await axios({
        method: "post",
        url: "http://3.18.143.100:5000/enrol",
        data: {
          emp_id: emp_id,
          emp_name: emp_name,
      ...
      if (onSubmit.status == 200) {
        alert(
          "You have successfully request to enroll in class " +
            e.target.value +
  ...
  async function getClassList() {
    try {
      const onSubmit = await axios({
        method: "post",
        url: "http://3.18.143.100:5000/all_eligible_classes",
              ...
              <ListItemText
                ...
                secondary={
                  <Typography
                    ...
                    color="text.primary"
                  >
                    {entry.start_enrol.slice(0, 16)} -{" "}
                    {entry.end_enrol.slice(0, 16)}
                  </Typography>
                }
              />
            </ListItem>
