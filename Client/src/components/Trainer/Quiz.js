...
function Quiz({ name, classID, quiz_id, currentHour, currentMin }) {
  ...
  async function handleSubmit() {
    ...
    if (name === "Final Quiz") {
      var URLink = "http://3.18.143.100:5002/create_final_quiz_question";
    } else {
      var URLink = "http://3.18.143.100:5002/create_question";
    }
    try {
  ...
  async function submitTime() {
    ...
    try {
      const onSubmit = await axios({
        method: "post",
        url: "http://3.18.143.100:5002/update_quiz_time",
        data: {
          quiz_id: quiz_id,
          time: duration,
        },
    ...
    <div className="quiz_details">
      {name !== "" && (
        <div className="section_contents">
          ...
          {name !== "Final Quiz" && (
            <div
              style={{ marginLeft: "10%", marginTop: "3%", textAlign: "left" }}
            >
