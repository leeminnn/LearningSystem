    ...
    async function getClassList() {
        try{
            const onSubmit =
              await axios({
                method: 'post',
                url: 'http://0.0.0.0:5000/all_classes',
            ...
            {classList.map(entry => (
                <div className='course_list'>
                    ...
                    <div className='button'>
                        <Button variant="outlined" value={entry.class_id} onClick={(e) => {history.push("/class/" + e.target.value); localStorage.setItem('intake', entry.intake)}}>View</Button>
                    </div>
                </div>
            ))}
        </div>
