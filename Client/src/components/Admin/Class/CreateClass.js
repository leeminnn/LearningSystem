...
function CreateClass() {
  ...
  const [inputFieldEnrol, setInputFieldEnrol] = useState([
    new Date(),
    new Date(),
  ]);
  ...
  const getClassList = () => {
    axios.get("http://3.18.143.100:5001/get_trainers").then((response) => {
  ...
  async function create() {
    ...
    try {
      const onSubmit = await axios({
        method: "post",
        url: "http://3.18.143.100:5000/create_class",
        data: data,
      });
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            <div style={{ width: "60%" }}>
              <Autocomplete
                ...
                onChange={(e, value) => {
                  setID(value.emp_id);
                  setlabelID(value.label);
                  setTrainerName(value.name);
                }}
                renderInput={(params) => (
                  <TextField {...params} label="Trainers" />
                )}
              />
            </div>
          </div>
